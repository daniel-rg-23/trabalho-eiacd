...
class Game:
    QUIET_TILES = [(0, 0), (2, 1), (2, 3), (1, 3)]
    MOVABLE_TILES = [(3, 2), (1, 2)]
    MATCH_TILES = [(0, 1), (0, 2)]
    ...
    def __init__(self):
        ...
        self.tiles_that_are_quiet = Game.QUIET_TILES
        self.tiles_that_move = Game.MOVABLE_TILES
        self.tiles_to_be_matched = Game.MATCH_TILES
    ...
    def solve_puzzle(self):
        ...
        # Use initial_state_matrix in your list comprehension
        initial_threes = [(r, c) for r, row in enumerate(initial_state_matrix) for c, val in enumerate(row) if val == 3]
        
        # Assuming goal_state_func and operators_func are defined somewhere
        solver = Algorithm(initial_state_matrix, goal_state_func, operators_func)
        ...
        if result_node:
            ...
            solution = list(path)  # Convert the generator to a list
            game_instance.apply_solution(solution)  # Pass the solution to apply_solution()
    ...
    def apply_solution(self, solution):
        ...
        # Update the game display with the initial state
        self.update_display()
    ...
    def update_display(self):
        ...
        # Create sprites for tiles based on the current game state
        for row in range(GAME_SIZE):
            for col in range(GAME_SIZE):
                tile_value = self.state_stack[-1][row][col]
                if tile_value != 0:
                    tile = Tile(self, col, row, str(tile_value), (255, 255, 255))  # Adjust text and color as needed
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if self.resart.collidepoint(mouse_pos):
                    ...
                    self.apply_solution(initial_state)  # Call the pop_state method to restore the previous game state
...
initial_state = game_instance.initial_state
initial_trees = game_instance.initial_threes
...
def move_tile(state, direction, row1, col1, row2, col2):
    ...
    if direction == 'up':
        ...
        while row1 > 0 and new_state[row1 - 1][col1] != 1:
            ...
            if (row1 - 1, col1) not in initial_trees:
        ...
        while row2 > 0 and new_state[row2 - 1][col2] != 1:
            ...
            if (row2 - 1, col2) not in initial_trees:
        ...
        while col1 > 0 and new_state[row1][col1 - 1] != 1:
            ...
            if (row1, col1 - 1) not in initial_trees:
                new_state[row1][col1] = 0
            col1 -= 1
            moved = True
        ...
        while col2 > 0 and new_state[row2][col2 - 1] != 1:
            ...
            if (row2, col2 - 1) not in initial_trees:
                new_state[row2][col2] = 0
            col2 -= 1
            moved = True
    ...
    if moved:
        for r, c in initial_trees:
            if new_state[r][c] != 2:  # If '2' is not over '3', make '3' appear
                new_state[r][c] = 3
        for r, c in [(r, c) for r in range(len(state)) for c in range(len(state[0])) if (r, c) not in initial_trees and new_state[r][c] == 3]:
            new_state[r][c] = 0
