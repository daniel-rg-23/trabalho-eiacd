...
class Game:
    ...
    def get_board_matrix(self):
        ...
        for row in range(GAME_SIZE):
            for col in range(GAME_SIZE):
                ...
                if (col, row) in self.tiles_that_are_quiet:
                    board[row][col] = 1  # Quiet tile
                elif (col, row) in self.tiles_that_move:
                    board[row][col] = 2  # Movable tile
                elif (col, row) in self.tiles_to_be_matched:
                    board[row][col] = 3  # Match tile
    ...
    def initial_state(self):
        ...
        # Return the board as the initial state
        return board


    def solve_puzzle(self):
        # Call the initial_state method to get the state
        ...
        solution_node = solver.breadth_first_search()

        # Extract the solution path
        solution_path = trace_path(solution_node)
        
        # Apply the solution to the game
        self.apply_solution(solution_path)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if self.resart.collidepoint(mouse_pos):
                    ...
                    self.solve_puzzle()  # Call the pop_state method to restore the previous game state
